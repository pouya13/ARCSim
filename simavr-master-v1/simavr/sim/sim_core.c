...
void pouya_instruction_decoder(char * inst)
{
	FILE * fp;
	fp = fopen("my_labels.txt", "a");
    fprintf(fp, "%s\n", inst);
    fclose(fp);
	// printf("%s\n", inst);
}

void pouya_instruction_decoder_sreg(char * inst, char num)
{
	FILE * fp;
	fp = fopen("my_labels.txt", "a");
    fprintf(fp, "%s", inst);
	...
	switch (opcode & 0xf000) {
				...
				default: {
					switch (opcode & 0xfc00) {
						case 0x0400: {	// CPC -- Compare with carry -- 0000 01rd dddd rrrr
							...
							snprintf(temp_buff, 15, "cpc");
							pouya_instruction_decoder(temp_buff);
						...
						case 0x0c00: {	// ADD -- Add without carry -- 0000 11rd dddd rrrr
							...
							if (r == d) {
								...
								snprintf(temp_buff, 15, "lsl");
								pouya_instruction_decoder(temp_buff);
							} else {
								STATE("add %s[%02x], %s[%02x] = %02x\n", avr_regname(d), vd, avr_regname(r), vr, res);
								snprintf(temp_buff, 15, "add");
								pouya_instruction_decoder(temp_buff);
								// pouya_instruction_decoder("add");
						...
						default:
							switch (opcode & 0xff00) {
								...
								case 0x0200: {	// MULS -- Multiply Signed -- 0000 0010 dddd rrrr
									...
									snprintf(temp_buff, 15, "muls");
									pouya_instruction_decoder(temp_buff);
									// pouya_instruction_decoder("muls_2");
								...
								case 0x0300: {	// MUL -- Multiply -- 0000 0011 fddd frrr
									...
									switch (opcode & 0x88) {
										case 0x00: 	// MULSU -- Multiply Signed Unsigned -- 0000 0011 0ddd 0rrr
											...
											c = (res >> 15) & 1;
											T(name = "mulsu";)
											snprintf(temp_buff, 15, "mulsu");
											pouya_instruction_decoder(temp_buff);
											// pouya_instruction_decoder("mulsu_2");
											break;
										case 0x08: 	// FMUL -- Fractional Multiply Unsigned -- 0000 0011 0ddd 1rrr
											...
											res <<= 1;
											T(name = "fmul";)
											snprintf(temp_buff, 15, "fmul");
											pouya_instruction_decoder(temp_buff);
											// pouya_instruction_decoder("fmul_2");
											break;
										case 0x80: 	// FMULS -- Multiply Signed -- 0000 0011 1ddd 0rrr
											...
											res <<= 1;
											T(name = "fmuls";)
											snprintf(temp_buff, 15, "fmuls");
											pouya_instruction_decoder(temp_buff);
											// pouya_instruction_decoder("fmuls_2");
											break;
										case 0x88: 	// FMULSU -- Multiply Signed Unsigned -- 0000 0011 1ddd 1rrr
											...
											res <<= 1;
											T(name = "fmulsu";)
											snprintf(temp_buff, 23, "fmulsu");
											pouya_instruction_decoder(temp_buff);
											// pouya_instruction_decoder("fmulsu_2");
											break;
									}
									cycle++;
		...
		case 0x1000: {
			switch (opcode & 0xfc00) {
				case 0x1800: {	// SUB -- Subtract without carry -- 0001 10rd dddd rrrr
					...
					snprintf(temp_buff, 15, "sub R%d, R%d", d, r);
					pouya_instruction_decoder(temp_buff);
					_avr_set_r(avr, d, res);
				...
				case 0x1000: {	// CPSE -- Compare, skip if equal -- 0001 00rd dddd rrrr
					...
					if (res){
						if (_avr_is_instruction_32_bits(avr, new_pc)){
							snprintf(temp_buff, 15, "cpse");
							pouya_instruction_decoder(temp_buff);
							pouya_instruction_decoder("add for cpse");
							pouya_instruction_decoder("nop");
							// for labels reading to detect the skip and place it with another one => if the triggered data recording is used
							// pouya_instruction_decoder("add");
							// pouya_instruction_decoder("nop");
							// pouya_instruction_decoder("cpse_2");
							// pouya_instruction_decoder("cpse_3");
						}
						else{
							snprintf(temp_buff, 15, "cpse");
							pouya_instruction_decoder(temp_buff);
				...
				case 0x1c00: {	// ADD -- Add with carry -- 0001 11rd dddd rrrr
					...
					if (r == d) {
						...
						snprintf(temp_buff, 15, "rol");
						pouya_instruction_decoder(temp_buff);
					} else {
						STATE("addc %s[%02x], %s[%02x] = %02x\n", avr_regname(d), avr->data[d], avr_regname(r), avr->data[r], res);
						snprintf(temp_buff, 15, "adc");
						pouya_instruction_decoder(temp_buff);
					}
					_avr_set_r(avr, d, res);
					_avr_flags_add_zns(avr, res, vd, vr);
			...
			switch (opcode & 0xfc00) {
				case 0x2000: {	// AND -- Logical AND -- 0010 00rd dddd rrrr
					...
					if (r == d) {
						...
						snprintf(temp_buff, 15, "tst");
						pouya_instruction_decoder(temp_buff);
					} else {
						STATE("and %s[%02x], %s[%02x] = %02x\n", avr_regname(d), vd, avr_regname(r), vr, res);
						snprintf(temp_buff, 15, "and");
						pouya_instruction_decoder(temp_buff);
					}
					_avr_set_r(avr, d, res);
				...
				case 0x2400: {	// EOR -- Logical Exclusive OR -- 0010 01rd dddd rrrr
					...
					if (r==d) {
						STATE("clr %s[%02x]\n", avr_regname(d), avr->data[d]);
						snprintf(temp_buff, 15, "clr");
						pouya_instruction_decoder(temp_buff);
					} else {
						STATE("eor %s[%02x], %s[%02x] = %02x\n", avr_regname(d), vd, avr_regname(r), vr, res);
						snprintf(temp_buff, 15, "eor");
						pouya_instruction_decoder(temp_buff);
					}
					_avr_set_r(avr, d, res);
					_avr_flags_znv0s(avr, res);
			...
			switch (opcode & 0xd008) {
				...
				case 0x8000: {	// LD (LDD) -- Load Indirect using Z -- 10q0 qqsd dddd yqqq
					...
					if (opcode & 0x0200) {
						...
						snprintf(temp_buff, 23, "std_z");
						pouya_instruction_decoder(temp_buff);
						// pouya_instruction_decoder("st_z+_2");
						_avr_set_ram(avr, v+q, avr->data[d]);
					} else {
						STATE("ld %s, (Z+%d[%04x])=[%02x]\n", avr_regname(d), q, v+q, avr->data[v+q]);
						snprintf(temp_buff, 23, "ldd_z");
						pouya_instruction_decoder(temp_buff);
						// pouya_instruction_decoder("ld_z+_2");
				...
				case 0x8008: {	// LD (LDD) -- Load Indirect using Y -- 10q0 qqsd dddd yqqq
					...
					if (opcode & 0x0200) {
						...
						snprintf(temp_buff, 23, "std_y");
						pouya_instruction_decoder(temp_buff);
						// pouya_instruction_decoder("st_y+_2");
						_avr_set_ram(avr, v+q, avr->data[d]);
					} else {
						STATE("ld %s, (Y+%d[%04x])=[%02x]\n", avr_regname(d), q, v+q, avr->data[d+q]);
						snprintf(temp_buff, 23, "ldd_y");
						pouya_instruction_decoder(temp_buff);
						// pouya_instruction_decoder("ld_y+_2");
						_avr_set_r(avr, d, _avr_get_ram(avr, v+q));
					}
					cycle += 1; // 2 cycles, 3 for tinyavr
				}	break;
		...
		case 0x9000: {
			...
			if ((opcode & 0xff0f) == 0x9408) {
				...
				if (opcode & 0x0080)
					pouya_instruction_decoder_sreg("sreg_cl_", _sreg_bit_name[b]);
				else
					pouya_instruction_decoder_sreg("sreg_se_", _sreg_bit_name[b]);
				avr_sreg_set(avr, b, (opcode & 0x0080) == 0);
				SREG();
			} else switch (opcode) {
				case 0x9588: { // SLEEP -- 1001 0101 1000 1000
					STATE("sleep\n");
					pouya_instruction_decoder("sleep");
				...
				case 0x9598: { // BREAK -- 1001 0101 1001 1000
					STATE("break\n");
					pouya_instruction_decoder("break");
					if (avr->gdb) {
				...
				case 0x9519: { // EICALL -- Indirect Call to Subroutine -- 1001 0101 0001 1001   bit 8 is "push pc"
					...
					STATE("%si%s Z[%04x]\n", e?"e":"", p?"call":"jmp", z << 1);
					if (p)
						pouya_instruction_decoder("icall");
					else
						pouya_instruction_decoder("ijmp");
					if (p)
				...
				case 0x95c8: {	// LPM -- Load Program Memory R0 <- (Z) -- 1001 0101 1100 1000
					...
					STATE("lpm %s, (Z[%04x])\n", avr_regname(0), z);
					pouya_instruction_decoder("lpm");
					// pouya_instruction_decoder("lpm_2");
					// pouya_instruction_decoder("lpm_3");
					cycle += 2; // 3 cycles
				...
				case 0x95d8: {	// ELPM -- Load Program Memory R0 <- (Z) -- 1001 0101 1101 1000
					...
					pouya_instruction_decoder("elpm");
					_avr_set_r(avr, 0, avr->flash[z]);
					cycle += 2; // 3 cycles
				}	break;
				default:  {
					switch (opcode & 0xfe0f) {
						case 0x9000: {	// LDS -- Load Direct from Data Space, 32 bits -- 1001 0000 0000 0000
							...
							pouya_instruction_decoder("lds");
							// pouya_instruction_decoder("lds_2");
						...
						case 0x9004: {	// LPM -- Load Program Memory -- 1001 000d dddd 01oo
							...
							STATE("lpm %s, (Z[%04x]%s)\n", avr_regname(d), z, op ? "+" : "");
							if (op){
								snprintf(temp_buff, 15, "lpm_z+");
								pouya_instruction_decoder(temp_buff);
								// pouya_instruction_decoder("lpm_z+_2");
								// pouya_instruction_decoder("lpm_z+_3");
							}
							else{
								snprintf(temp_buff, 15, "lpm_z");
								pouya_instruction_decoder(temp_buff);
								// pouya_instruction_decoder("lpm_z_2");
								// pouya_instruction_decoder("lpm_z_3");
							}
						...
						case 0x9007: {	// ELPM -- Extended Load Program Memory -- 1001 000d dddd 01oo
							...
							if (op)
								pouya_instruction_decoder("elpm_z+");
							else
								pouya_instruction_decoder("elpm_z");
							_avr_set_r(avr, d, avr->flash[z]);
							if (op) {
						...
						case 0x900e: {	// LD -- Load Indirect from Data using X -- 1001 000d dddd 11oo
							...
							if (op==1){
								snprintf(temp_buff, 23, "ld_x+");
								pouya_instruction_decoder(temp_buff);	
								// pouya_instruction_decoder("ld_x++_2");
							}
							else if (op==2){
								snprintf(temp_buff, 23, "ld_x-");
								pouya_instruction_decoder(temp_buff);
								...
								snprintf(temp_buff, 23, "ld_x");
								pouya_instruction_decoder(temp_buff);
								// pouya_instruction_decoder("ld_x_2");
							}
						...
						case 0x920e: {	// ST -- Store Indirect Data Space X -- 1001 001d dddd 11oo
							...
							if (op==1){
								snprintf(temp_buff, 23, "st_x+");
								pouya_instruction_decoder(temp_buff);	
								// pouya_instruction_decoder("st_x++_2");
							}
							else if (op==2){
								snprintf(temp_buff, 23, "st_x-");
								pouya_instruction_decoder(temp_buff);	
								...
								snprintf(temp_buff, 23, "st_x");
								pouya_instruction_decoder(temp_buff);	
								// pouya_instruction_decoder("st_x_2");
							}
						...
						case 0x900a: {	// LD -- Load Indirect from Data using Y -- 1001 000d dddd 10oo
							...
							if (op==1){
								...
								snprintf(temp_buff, 23, "ld_y");
								pouya_instruction_decoder(temp_buff);	
								// pouya_instruction_decoder("ld_y_2");
							}
							cycle++; // 2 cycles, except tinyavr
							if (op == 2) y--;
						...
						case 0x920a: {	// ST -- Store Indirect Data Space Y -- 1001 001d dddd 10oo
							...
							if (op==1){
								...
								snprintf(temp_buff, 23, "st_y");
								pouya_instruction_decoder(temp_buff);
								// pouya_instruction_decoder("st_y_2");
							}
							cycle++;
							if (op == 2) y--;
						...
						case 0x9200: {	// STS -- Store Direct to Data Space, 32 bits -- 1001 0010 0000 0000
							...
							snprintf(temp_buff, 15, "sts");
							pouya_instruction_decoder(temp_buff);
							// pouya_instruction_decoder("sts_2");
							cycle++;
							_avr_set_ram(avr, x, vd);
						}	break;
						case 0x9001:
						case 0x9002: {	// LD -- Load Indirect from Data using Z -- 1001 000d dddd 00oo
							...
							if (op==1){
								...
								snprintf(temp_buff, 23, "ld_z");
								pouya_instruction_decoder(temp_buff);
								// pouya_instruction_decoder("ld_z_2");
							}
							cycle++;; // 2 cycles, except tinyavr
							if (op == 2) z--;
						...
						case 0x9202: {	// ST -- Store Indirect Data Space Z -- 1001 001d dddd 00oo
							...
							if (op==1){
								...
								snprintf(temp_buff, 15, "st_z");
								pouya_instruction_decoder(temp_buff);
								// pouya_instruction_decoder("st_z_2");
							}
							cycle++; // 2 cycles, except tinyavr
							if (op == 2) z--;
						...
						case 0x940f: {	// CALL -- Long Call to sub, 32 bits -- 1001 010a aaaa 111a
							...
							a = (a << 16) | x;
							STATE("call 0x%06x\n", a);
							pouya_instruction_decoder("call");
							new_pc += 2;
						...
						default: {
							switch (opcode & 0xff00) {
								case 0x9600: {	// ADIW -- Add Immediate to Word -- 1001 0110 KKpp KKKK
									...
									snprintf(temp_buff, 23, "adiw");
									pouya_instruction_decoder(temp_buff);
									// pouya_instruction_decoder("adiw_2");
								...
								case 0x9700: {	// SBIW -- Subtract Immediate from Word -- 1001 0111 KKpp KKKK
									...
									snprintf(temp_buff, 23, "sbwi");
									pouya_instruction_decoder(temp_buff);
									// pouya_instruction_decoder("sbiw_2");
									_avr_set_r16le_hl(avr, p, res);
									avr->sreg[S_V] = ((vp & ~res) >> 15) & 1;
									avr->sreg[S_C] = ((res & ~vp) >> 15) & 1;
									_avr_flags_zns16(avr, res);
								...
								case 0x9800: {	// CBI -- Clear Bit in I/O Register -- 1001 1000 AAAA Abbb
									...
									snprintf(temp_buff, 23, "cbi");
									pouya_instruction_decoder(temp_buff);
									// pouya_instruction_decoder("cbi_2");
								...
								case 0x9900: {	// SBIC -- Skip if Bit in I/O Register is Cleared -- 1001 1001 AAAA Abbb
									...
									if (!res){
										if (_avr_is_instruction_32_bits(avr, new_pc)){
											snprintf(temp_buff, 23, "sbic");
											pouya_instruction_decoder(temp_buff);

											// pouya_instruction_decoder("sbic");
											pouya_instruction_decoder("add for sbic");
											pouya_instruction_decoder("nop");
											// pouya_instruction_decoder("sbic_2");
											// pouya_instruction_decoder("sbic_3");
										}
										else{
											snprintf(temp_buff, 23, "sbic");
											pouya_instruction_decoder(temp_buff);
								...
								case 0x9a00: {	// SBI -- Set Bit in I/O Register -- 1001 1010 AAAA Abbb
									...
									snprintf(temp_buff, 23, "sbi_1");
									pouya_instruction_decoder(temp_buff);
									// pouya_instruction_decoder("sbi_2");
									_avr_set_ram(avr, io, res);
									cycle++;
								}	break;
								case 0x9b00: {	// SBIS -- Skip if Bit in I/O Register is Set -- 1001 1011 AAAA Abbb
									...
									if (res){
										if (_avr_is_instruction_32_bits(avr, new_pc)){
											snprintf(temp_buff, 23, "sbis");
											pouya_instruction_decoder(temp_buff);
											pouya_instruction_decoder("add for sbis");
											pouya_instruction_decoder("nop");
											// pouya_instruction_decoder("sbis_2");
											// pouya_instruction_decoder("sbis_3");
										}
										else{
											snprintf(temp_buff, 23, "sbis");
											pouya_instruction_decoder(temp_buff);
								...
								default:
									switch (opcode & 0xfc00) {
										case 0x9c00: {	// MUL -- Multiply Unsigned -- 1001 11rd dddd rrrr
											...
											snprintf(temp_buff, 23, "mul");
											pouya_instruction_decoder(temp_buff);
											// pouya_instruction_decoder("mul_2");
											cycle++;
											_avr_set_r16le(avr, 0, res);
											avr->sreg[S_Z] = res == 0;
											avr->sreg[S_C] = (res >> 15) & 1;
		...
		case 0xc000: {	// RJMP -- 1100 kkkk kkkk kkkk
			get_o12(opcode);
			STATE("rjmp .%d [%04x]\n", o >> 1, new_pc + o);
			pouya_instruction_decoder("rjmp");
		...
		case 0xd000: {	// RCALL -- 1101 kkkk kkkk kkkk
			get_o12(opcode);
			STATE("rcall .%d [%04x]\n", o >> 1, new_pc + o);
			pouya_instruction_decoder("rcall");
		...
		case 0xe000: {	// LDI Rd, K aka SER (LDI r, 0xff) -- 1110 kkkk dddd kkkk
			get_h4_k8(opcode);
			STATE("ldi %s, 0x%02x\n", avr_regname(h), k);
			snprintf(temp_buff, 15, "ldi");
			pouya_instruction_decoder(temp_buff);
			// pouya_instruction_decoder("ldi");
			_avr_set_r(avr, h, k);
		}	break;
			...
			switch (opcode & 0xfe00) {
				...
				case 0xf600: {	// BRXC/BRXS -- All the SREG branches -- 1111 0Boo oooo osss
					...
					if (names[set][s]) {
						...
						// pouya_instruction_decoder(names[set][s]);
						snprintf(temp_buff, 15, "%s", names[set][s]);
						pouya_instruction_decoder(temp_buff);
						// printf("##############\n");
						// printf("%s\n", names[set][s]);
						// printf("%d %d\n", set, s);
						...
						if (set)
							pouya_instruction_decoder_sreg("brbs_", _sreg_bit_name[s]);
						else
							pouya_instruction_decoder_sreg("brbc_", _sreg_bit_name[s]);
					}
					if (branch) {
				...
				case 0xfe00: {	// SBRS/SBRC -- Skip if Bit in Register is Set/Clear -- 1111 11sd dddd 0bbb
					...
					if (set){
						if (branch){
							if (_avr_is_instruction_32_bits(avr, new_pc)){
								snprintf(temp_buff, 23, "sbrs");
								pouya_instruction_decoder(temp_buff);
								// pouya_instruction_decoder("sbrs");
								pouya_instruction_decoder("add for sbrs");
								pouya_instruction_decoder("nop");
								// pouya_instruction_decoder("sbrs_2");
								// pouya_instruction_decoder("sbrs_3");
							}
							else{
								snprintf(temp_buff, 23, "sbrs");
								pouya_instruction_decoder(temp_buff);
						...
						if (branch){
							if (_avr_is_instruction_32_bits(avr, new_pc)){
								snprintf(temp_buff, 23, "sbrc");
								pouya_instruction_decoder(temp_buff);
								pouya_instruction_decoder("add for sbrc");
								pouya_instruction_decoder("nop");
								// pouya_instruction_decoder("sbrc_2");
								// pouya_instruction_decoder("sbrc_3");
							}
							else{
								snprintf(temp_buff, 23, "sbrc");
								pouya_instruction_decoder(temp_buff);
